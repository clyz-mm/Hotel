...
public class DB {
	...
	public static Connection getCon() {// 得到数据库连接的方法
		/*
		 * try{ Context initial = new InitialContext();//得到上下文引用 DataSource ds =
		 * //得到DataSource引用
		 * (DataSource)initial.lookup("java:comp/env/jdbc/jiudian"); connection
		 * = ds.getConnection();//得到数据库连接 } catch(Exception e)
		 * {e.printStackTrace();} return connection;//返回数据库连接
		 */
		try {
			Class.forName("com.mysql.jdbc.Driver");
			String url = "jdbc:mysql://localhost/hotel";
			con = DriverManager.getConnection(url, "root", "harry123");
	...
	//*******************得到某张表某一列的最大值并加1***************************
		...
		public static int update(String sqla){
			...
			try{
				con = DB.getCon();//得到数据库连接
				stat = con.createStatement();//创建语句对象
				changedCount = stat.executeUpdate(sqla);//进行更新
			}
			catch(Exception e)	{e.printStackTrace();}
			finally {DB.closeCon();}//关闭数据库连接 		
